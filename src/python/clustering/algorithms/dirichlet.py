...
def dirichlet_em(X_all, K, alpha=1e-3, init=default_init, seed=None,
  ...
  pa.alpha = 1e-3*np.ones(K,)
  pa.m, pa.W = _init_means_and_cov(X_all, K, hp, seed=seed)
  ...
  for epoch in xrange(max_epoch):
    for i, (n, batch) in enumerate(gen_batches(n_all, n_minibatch)):
      ...
      nudxWdx = pa.nu.reshape((K,1)).astype(np.float32) * dxWdx

      logp = 0.5*( expLogDetLam.reshape((K,1)).astype(np.float32) 
                   + 2*expLogPi.reshape((K,1)).astype(np.float32)
                   - nudxWdx 
                   - d/pa.beta.reshape((K,1)).astype(np.float32)
                  ) # (K,n)
      ...
      dv.alpha = hp.alpha0 + Nk - pa.alpha
      ...
      llik_new = _bound(logp, pa, hp, Nk, Xk, Sk)
      diff = llik_new - llik
...
def _init_means_and_cov(X, K, hp, seed=None):
  d = X.shape[0]
  kmeans = cluster.KMeans(n_clusters=K, verbose=False, random_state=seed)
  kmeans.fit(X.T)
...
def _bound(logp, pa, hp, Nk, Xk, Sk):
  d, K = Xk.shape
  pk = Nk
  ...
  # Exp[ log p(Z | pi) ]
  expLogpZ = np.sum(pk * expLogPi, axis=0)

  # Exp[ log p(pi) ]
  expLogpPi = _log_inv_dirichlet_Z(hp.alpha0 * np.ones(K,)) \
            + (hp.alpha0 - 1) * np.sum(expLogPi)

  # Exp[ log q(Z) ]
  expLogqZ = np.sum(np.exp(logp) * logp)

  # Exp[ log q(pi) ]
  expLogqPi = _log_inv_dirichlet_Z(pa.alpha) + np.sum((pa.alpha - 1) * expLogPi,axis=0)
  ...
  # compute bound
  llik_new = expLogpX + expLogpZ + expLogpMuLam + expLogpPi \
           - expLogqZ - expLogqMuLam - expLogqPi
  return llik_new
...
def _expLogPi(alpha):
  """Expectation of log(pi) under q(pi|alpha)."""
  return np.array([psi(a) for a in alpha]) \
         - psi(np.sum(alpha))
...
def _log_inv_dirichlet_Z(alpha):
  """Inverse log normalizing constant for dirichlet distribution."""
  return gammaln(np.sum(alpha)) - np.sum([gammaln(a) for a in alpha])
